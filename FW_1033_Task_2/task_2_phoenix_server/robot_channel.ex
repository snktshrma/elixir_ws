defmodule Task2PhoenixServerWeb.RobotChannel do
  ...
  def handle_in("new_msg", message, socket) do
    mapp =  %{"a" => 1, "b" => 2, "c" => 3, "d" => 4, "e" => 5}
    msg = %{ "left" => (message["x"]-1)*150, "bottom" => (mapp[message["y"]]-1)*150, "face" => message["face"]}
    :ok = Phoenix.PubSub.subscribe(Task2PhoenixServer.PubSub, "robot:update")
    :ok = Phoenix.PubSub.broadcast(Task2PhoenixServer.PubSub, "robot:update", msg)
    ###########################
