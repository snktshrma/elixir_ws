defmodule ToyRobot do
  ...
  def for_loop(error_y,robot,error_x) when error_y == 1 and error_x == 1 do
    robot = move(robot)
    send_robot_status(robot, :cli_robot_state)
    {:ok, %ToyRobot.Position{x: robot.x, y: robot.y, facing: robot.facing}}
  end 

  def for_loop(error_y,robot,error_x) when error_y == 1 and 0 <= error_x and error_x <= 1 do
    robot = move(robot)
    send_robot_status(robot, :cli_robot_state)
    {:ok, %ToyRobot.Position{x: robot.x, y: robot.y, facing: robot.facing}}
  end 

  def for_loop(error_y, robot,error_x) when error_y <= 1 do
    robot = if error_y == 1 do
              robot = move(robot)
              send_robot_status(robot, :cli_robot_state)
              robot
            else
    ...
    cond do
      error_x > 0 and @vals_facing[robot.facing] == 2 -> for_loop(error_x,robot,error_y)
      error_x < 0 and @vals_facing[robot.facing] == 4 -> for_loop(error_x*(-1),robot,error_y)
      error_x > 0 ->
        robot = %{robot | facing: :east}
        send_robot_status(robot, :cli_robot_state)
        for_loop(error_x,robot,error_y)
      error_x < 0 ->
        robot = %{robot | facing: :west}
        send_robot_status(robot, :cli_robot_state)
        for_loop(error_x*(-1),robot,error_y)
      error_x == 0 -> for_loop(error_x, robot, error_y)
    end
  end

  def for_loop(error_y,robot,error_x) do
    robot = move(robot)
    send_robot_status(robot, :cli_robot_state)
    for_loop(error_y-1, robot,error_x)
  end
  ...
  def stop(robot, goal_x, goal_y, cli_proc_name) do
    ...
    cond do
      ...
      error_y > 0 and @vals_facing[robot.facing] == 1 -> for_loop(error_y,robot,error_x)
      error_y < 0 and @vals_facing[robot.facing] == 3 -> for_loop(error_y*(-1),robot,error_x)
      error_y > 0 ->
        robot = %{robot | facing: :north}
        send_robot_status(robot, :cli_robot_state)
        for_loop(error_y,robot,error_x)
      error_y < 0 ->
